...
LD R6, SUB_TO_UPPER 					;Put SUB_TO_UPPER location in R6
JSRR R6 						;Go to x3600 for SUB_TO_UPPER
...
CHECK_PALINDROME
	...
	TRUE
		...
		BRnz POST_TRUE				;If they did, go to DONE
...
LD R1, UPPER_CASE 					;Store x20 in R1
NOT R1, R1 						;R1 <-- NOT(x20)

DO_WHILE_LOOP_2
	LDR R2, R0, #0 					;R2 <-- Mem[R0]
	AND R2, R2, R1 					;R2 <-- AND R2 with NOT(x20)
	STR R2, R0, #0 					;R0 <-- Mem[R2]
	ADD R0, R0, #1 					;Increment pointer value
	LDR R3, R0, #0					;Check if 0
	BRz FINISH 					;Go to FINISH if end of array
	BR DO_WHILE_LOOP_2
FINISH
